(function(){
  ...
  function removeStepEventDuplicates(s){
    for(const celebration of s.celebrations||[]){
      for(const day of celebration.days||[]){
        if(!Array.isArray(day.contentIds)||!day.contentIds.length)continue;
        const eventContentIds=new Set((s.events||[]).filter(e=>e.celebrationId===celebration.id&&e.dayKey===day.key).flatMap(e=>e.contentIds||[]).map(String));
        if(!eventContentIds.size)continue;
        day.contentIds=day.contentIds.filter(id=>!eventContentIds.has(String(id)));
      }
    }
  }
