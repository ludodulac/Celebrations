...
function galleryOriginMeta(c){
  ...
  const origin=locations.length?locations.join(' · '):(c.category||'');
...
function renderLibrary(){
  ...
  library.innerHTML=`<div class="library-fixed-head"><div class="section-head"><div><div class="eyebrow">Médiathèque</div><h2>Contenus</h2></div></div><div class="library-main-tabs"><button class="btn ${libraryFamily==='audio'?'primary':''}" data-library-family="audio">Tous les audios</button><button class="btn ${libraryFamily==='video'?'primary':''}" data-library-family="video">Toutes les vidéos</button><button class="btn ${libraryFamily==='text'?'primary':''}" data-library-family="text">Tous les textes</button><button class="btn ${libraryFamily==='image'?'primary':''}" data-library-family="image">Toutes les images</button><button class="btn ${libraryFamily==='pdf'?'primary':''}" data-library-family="pdf">Tous les PDF</button></div>${audioSubs}</div><div class="library-scroll"><div class="grid3" ${libraryFamily==='image'?'data-image-gallery-grid':''}>${cards}</div></div>`;
